...
function stopRecording() {
    if (mediaRecorder && isRecording) {
        ...
        console.log('Recording stopped, sending to transcription API');
    }
}
...
async function transcribeAudio(audioBlob) {
    try {
        ...
        // Deepgram accepts WebM directly - no conversion needed!
        console.log('Sending WebM to Deepgram API...');
        ...
        reader.onloadend = async () => {
            ...
            // Call our transcription API endpoint
...
// ================================================================
// JAVASCRIPT ALIGNMENT ENFORCER
// Add this to the END of your app.js file
// Forces card alignment even if CSS fails
// ================================================================

/**
 * Force card alignment using JavaScript
 * This runs after page load and overrides any CSS issues
 */
function forceCardAlignment() {
    console.log('🔧 Forcing card alignment...');
    
    // Get the container and panels
    const container = document.querySelector('.dashboard-container');
    const leftPanel = document.querySelector('.panel-left');
    const rightPanel = document.querySelector('.panel-right');
    
    if (!container || !leftPanel || !rightPanel) {
        console.warn('⚠️ Could not find dashboard elements');
        return;
    }
    ...
    // Force panel alignment
    leftPanel.style.alignSelf = 'start';
    leftPanel.style.marginTop = '0';
    leftPanel.style.paddingTop = '0';
    
    rightPanel.style.alignSelf = 'start';
    rightPanel.style.marginTop = '0';
    rightPanel.style.paddingTop = '0';
    
    // Force all cards to have zero top margin
    ...
    // Ensure first card in each panel starts at top
    const leftCards = leftPanel.querySelectorAll('.card');
    const rightCards = rightPanel.querySelectorAll('.card');
    
    if (leftCards.length > 0) {
        leftCards[0].style.marginTop = '0';
        leftCards[0].style.paddingTop = '0';
    }
    
    if (rightCards.length > 0) {
        rightCards[0].style.marginTop = '0';
        rightCards[0].style.paddingTop = '0';
    }
    
    console.log('✅ Card alignment forced!');
}

/**
 * Diagnostic function - shows current alignment status
 */
function diagnoseAlignment() {
    console.log('🔍 ALIGNMENT DIAGNOSTICS');
    console.log('========================');
    ...
    if (leftPanel) {
        ...
        console.log('\nLeft Panel:');
        console.log('  align-self:', leftStyles.alignSelf);
        console.log('  margin-top:', leftStyles.marginTop);
        console.log('  top position:', leftRect.top);
    }
    ...
    if (rightPanel) {
        ...
        console.log('\nRight Panel:');
        console.log('  align-self:', rightStyles.alignSelf);
        console.log('  margin-top:', rightStyles.marginTop);
        console.log('  top position:', rightRect.top);
    }
    
    // Check first cards in each panel
    const leftFirstCard = leftPanel?.querySelector('.card');
    const rightFirstCard = rightPanel?.querySelector('.card');
    
    if (leftFirstCard) {
        const leftCardRect = leftFirstCard.getBoundingClientRect();
        console.log('\nLeft First Card:');
        console.log('  top position:', leftCardRect.top);
    }
    
    if (rightFirstCard) {
        const rightCardRect = rightFirstCard.getBoundingClientRect();
        console.log('\nRight First Card:');
        console.log('  top position:', rightCardRect.top);
    }
    
    // Calculate misalignment
    if (leftFirstCard && rightFirstCard) {
        const leftTop = leftFirstCard.getBoundingClientRect().top;
        const rightTop = rightFirstCard.getBoundingClientRect().top;
        const diff = Math.abs(leftTop - rightTop);
        
        console.log('\n📏 ALIGNMENT CHECK:');
        console.log('  Left card top:', leftTop.toFixed(2));
        console.log('  Right card top:', rightTop.toFixed(2));
        console.log('  Difference:', diff.toFixed(2), 'pixels');
        
        if (diff < 1) {
            console.log('  ✅ Cards are ALIGNED!');
        } else {
            console.log('  ❌ Cards are MISALIGNED by', diff.toFixed(2), 'pixels');
        }
    }
...
function initializeAlignment() {
    ...
    // Run after a short delay (in case of late-loading content)
    setTimeout(forceCardAlignment, 100);
    setTimeout(forceCardAlignment, 500);
    setTimeout(forceCardAlignment, 1000);
    
    // Run diagnostic after everything loads
    setTimeout(diagnoseAlignment, 1100);
}

// Execute on DOM ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeAlignment);
} else {
    initializeAlignment();
}
...
window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(forceCardAlignment, 250);
});

// Export for manual testing in console
window.forceCardAlignment = forceCardAlignment;
window.diagnoseAlignment = diagnoseAlignment;

console.log('💡 Alignment fixer loaded. Type diagnoseAlignment() in console to check status.');
